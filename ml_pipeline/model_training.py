...
def carrega_dados_feature_store(feature_store_path, target):

    # Extrai os dados da feature store
    feature_store = FeatureStore(feature_store_path)

    # Carrega os atributos
    features = feature_store.carrega_atributos()
...
def treina_modelo(target, model_output_path):

    # Carrega os dados da feature store
    features, labels = carrega_dados_feature_store(FEATURE_STORE_PATH, target)
